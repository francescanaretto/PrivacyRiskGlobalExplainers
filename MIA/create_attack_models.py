...
def merge_partial_attack_dataset(under: float = None) -> List:
    ...
    for label, df in attack_dataset.groupby("class_label", sort=False):
        ...
        # Saves data on disk
        os.makedirs(
            f"{HOMEDIR}/attack_{data_kind}_{kind}_{classifier}/{BLACK_BOX_TYPE}_{SHADOW_TYPE}_label_{label}",
            exist_ok=True,
        )
        savez_compressed(
    ...
    #print('labelssss from merge dataset ', labels)
    return labels
...
def train_attack_model(label: int):
    ...
    with open(
        f"{HOMEDIR}/attack_{data_kind}_{kind}_{classifier}/{BLACK_BOX_TYPE}_{SHADOW_TYPE}_label_{label}/train_report.txt",
        "w",
    ) as f:
        f.write(train_report)
    with open(
        f"{HOMEDIR}/attack_{data_kind}_{kind}_{classifier}/{BLACK_BOX_TYPE}_{SHADOW_TYPE}_label_{label}/test_report.txt",
        "w",
    ) as f:
        f.write(test_report)
